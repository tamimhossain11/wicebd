...
export default function LFRMazeSolvingRegistrationForm({ onPromoChange }) {
  const { user } = useAuth();
          ...
          {step === 2 && (
            <motion.div key="s2" variants={stepVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', mb: 3.5 }}>
                <TextField
                  value={promoInput}
                  onChange={(e) => { setPromoInput(e.target.value.toUpperCase()); setPromoStatus(null); onPromoChange?.({ code: '', discount: 0 }); }}
