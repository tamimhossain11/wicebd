...
router.post('/attendance/:cardUid/certificate', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    if (card.registration_type === 'guest') {
      return res.status(400).json({ success: false, message: 'Certificate collection is not available for guest cards' });
    }
...
router.get('/judges', authenticateAdmin, requireRole('super_admin'), async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, name, username, email, judge_type, subcategory, education_level, display_password, is_active, created_at FROM judges ORDER BY id DESC'
    );
